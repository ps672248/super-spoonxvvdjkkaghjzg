...
export default function LeaderboardScreen() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const value = myValueFor(sessions, metric, tier);
    setMyValue(value);
    try {
      const [board, rank, target] = await Promise.all([
        fetchBoard(metric, tier),
        user ? fetchMyRank(metric, tier, value) : Promise.resolve(0),
        user ? fetchOvertakeTarget(metric, tier, value) : Promise.resolve(null),
      ]);
      setEntries(board);
      setMyRank(rank);
      setOvertake(target);
