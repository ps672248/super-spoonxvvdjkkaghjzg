...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('college_bot.log', encoding='utf-8'),
...
TOKEN_FILE     = 'college_token.json'
HISTORY_FILE   = 'college_history.json'
...
# re-comment/re-reply. Keeps college_history.json from growing forever.
...
SEARCH_QUERIES = [
    # ── BCA / MCA ─────────────────────────────────────────────
    'bca semester exam preparation',
    'mca semester exam preparation',
    'dbms exam preparation bca mca',
    'data structures exam preparation semester',
    'programming subject semester exam tips',

    # ── Medical / paramedical ─────────────────────────────────
    'mbbs first year anatomy preparation',
    'mbbs physiology exam preparation',
    'nursing exam preparation semester',
    'bpt physiotherapy exam preparation',
    'b pharm pharmacology exam preparation',
    'bmlt exam preparation semester',
    'nursing viva preparation tips',

    # ── Cross-cutting / Hinglish ──────────────────────────────
    'university semester exam kaise clear kare',
    'college exam preparation last minute',
    'semester exam ki taiyari kaise kare',
]
...
Purpose: Free MCQ practice for college degree students — BCA, MCA, MBBS, BMLT/BMLS, BMRIT, BPT, B.Sc Nursing, B.Pharm

Key features:
1. Gamified MCQ modes: MCQ Blitz, Survival (timed, limited lives), Match the Following, Syllabus Slasher
2. Subject-wise core-curriculum practice authored from each degree's real official syllabus (NMC CBME for MBBS, INC curriculum for Nursing, PCI regulations for B.Pharm, RGUHS/university curricula for the rest)
3. Insights dashboard — accuracy tracking per topic so you know exactly what to revise before the exam
...
Target users: College students in BCA, MCA, MBBS, BMLT/BMLS, BMRIT, BPT, B.Sc Nursing, B.Pharm preparing for semester exams
...
def load_history():
    if os.path.exists(HISTORY_FILE):
        try:
            ...
            log.warning("college_history.json corrupt — starting fresh")
...
VIDEO_KEYWORDS = [
    # Degrees
    'bca', 'mca', 'mbbs', 'bmlt', 'bmls', 'bmrit', 'bpt', 'physiotherapy',
    'b.sc nursing', 'bsc nursing', 'b.pharm', 'bpharm', 'pharmacy student',

    # Subjects
    'anatomy', 'physiology', 'pharmacology', 'dbms', 'data structures',
    'operating system', 'computer networks', 'programming exam',
    'nursing procedure', 'nursing foundation', 'microbiology exam',
    'biochemistry exam', 'pathology exam',

    # Exam terms
    'semester exam', 'university exam', 'internal exam', 'viva preparation',
    'practical exam', 'exam pattern', 'previous year paper', 'important questions',
    'first year exam', 'second year exam', 'third year exam', 'final year exam',

    # Hinglish
    'semester exam ki taiyari', 'exam kaise clear kare', 'last minute revision',
]

# ── COMMENT KEYWORDS (for scanning replies) ──────────────
COMMENT_KEYWORDS = [
    'prepare', 'preparation', 'how to prepare', 'how to score', 'how to clear',
    'suggest', 'resource', 'material', 'study material', 'notes',
    'study', 'practice', 'mock test', 'previous year', 'important questions',
    'tips', 'help', 'guide', 'recommend', 'strategy', 'plan',
    'syllabus', 'pattern', 'book', 'reference', 'pdf', 'viva',
    ...
    'first time', 'no idea', 'any idea', 'fail hone ka dar',
    'kaise', 'kya', 'batao', 'suggest karo', 'koi bata',
    'padhai', 'taiyari', 'clear kaise', 'koi app', 'best resource',
...
def is_video_relevant(title: str, description: str) -> tuple[bool, str]:
    ...
    return False, 'no college-degree keywords found'
...
def generate_top_comment(video_title: str, video_description: str) -> str:
    ...
    if mention_app:
        prompt = f"""Write a genuine YouTube comment from a college student (BCA/MCA/MBBS/Nursing/BPT/B.Pharm) who naturally mentions using an app for MCQ practice.
...
Write ONLY the comment text, nothing else."""
        ...
        prompt = f"""Write a genuine YouTube comment from a college student (BCA/MCA/MBBS/Nursing/BPT/B.Pharm) watching this video.

Video title: {video_title}
Video description (first 400 chars): {video_description[:400]}

Rules:
- Comment must be SPECIFIC to the topic in this video — mention the subject or concept
- Share a real study tip, ask a genuine question about the topic, or relate a personal experience with this subject
- Sound like a real college student, NOT a bot
...
def generate_reply(comment_text: str, video_title: str) -> str:
    ...
    if mention_app:
        prompt = f"""Write a helpful reply to this YouTube comment from a fellow college student who uses Aspirant Arcade for MCQ practice.
...
Write ONLY the reply text, nothing else."""
        ...
        prompt = f"""Write a helpful, genuine reply to this YouTube comment from a fellow college student.
...
- Sound like a fellow BCA/MCA/MBBS/Nursing/BPT/B.Pharm student, not a tutor or promoter
- 1-2 sentences, casual, Hinglish OK
