...
import { Category, CATEGORIES, getCategory } from '../config/categories';
...
interface ConfigState {
  ...
  getCategory: (id?: string | null) => Category;
}

const STORAGE_KEYS = {
  PSUS: 'psuplus_master_psus',
  BRANCHES: 'psuplus_master_branches',
  CATEGORIES: 'psuplus_master_categories',
...
export const useConfigStore = create<ConfigState>((set, get) => ({
  ...
  loadConfig: async () => {
    ...
    try {
      ...
      const [localPsus, localBranches, localCategories, localVersion] = await Promise.all([
      ...
      if (localPsus && localBranches) {
        set({
          ...
          categories: localCategories ? JSON.parse(localCategories) : CATEGORIES,
          isLoaded: true,
          isLoading: false,
        });
      } else {
      ...
      if (remoteVersion > CONFIG_VERSION) {
        ...
        const [psuSnap, branchSnap, categorySnap] = await Promise.all([
        ...
        const freshCategories = categorySnap.docs.map(d => ({ id: d.id, ...d.data() })) as Category[];

        if (freshPsus.length > 0 && freshBranches.length > 0) {
          const categoriesToSave = freshCategories.length > 0 ? freshCategories : CATEGORIES;
          await Promise.all([
            ...
            AsyncStorage.setItem(STORAGE_KEYS.CATEGORIES, JSON.stringify(categoriesToSave)),
            AsyncStorage.setItem(STORAGE_KEYS.VERSION, remoteVersion.toString()),
          ]);
          set({ psus: freshPsus, branches: freshBranches, categories: categoriesToSave, isLoaded: true });
        }
        ...
        set({ psus: PSUS, branches: BRANCHES, categories: CATEGORIES, isLoaded: true });
      }
  ...
  getCategory: (id) => getCategory(id, get().categories),
}));
