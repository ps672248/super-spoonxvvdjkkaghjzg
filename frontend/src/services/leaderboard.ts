...
// Boards are namespaced by category so PSU and Schooling aspirants compete on
// separate boards: leaderboards/{category}_{metric}_{window}/scores/{uid}
function boardId(category: string, metric: BoardMetric, window: BoardWindow): string {
  return `${category}_${metric}_${windowToken(window)}`;
}

function scoresCol(category: string, metric: BoardMetric, window: BoardWindow) {
  return collection(db, 'leaderboards', boardId(category, metric, window), 'scores');
}

/** Sessions played on exams that belong to the given category. */
function sessionsForCategory(sessions: StudySession[], category: string): StudySession[] {
  const cats = useConfigStore.getState().categories;
  return sessions.filter(s => categoryIdForExam(s.psuId, cats) === category);
}
...
export async function upsertLeaderboard(
  ...
  if (SCORE_BOARDS.includes(playedMode)) {
    const best = bestScore(scoped, playedMode, startMs);
    batch.set(doc(scoresCol(category, playedMode, 'week'), uid), { ...meta, value: best.week });
    batch.set(doc(scoresCol(category, playedMode, 'alltime'), uid), { ...meta, value: best.all });
  }
...
export async function syncAllLeaderboards(
  ...
  const cats = useConfigStore.getState().categories;
  const categoryIds = new Set(sessions.map(s => categoryIdForExam(s.psuId, cats)));
  const batch = writeBatch(db);
  ...
  for (const category of categoryIds) {
    ...
    for (const mode of SCORE_BOARDS) {
      const best = bestScore(scoped, mode, startMs);
      if (best.all <= 0) continue; // never played this mode → no entry
      batch.set(doc(scoresCol(category, mode, 'week'), uid), { ...meta, value: best.week });
      batch.set(doc(scoresCol(category, mode, 'alltime'), uid), { ...meta, value: best.all });
    }
  }
...
export async function fetchBoard(
  ...
  const q = query(scoresCol(category, metric, window), orderBy('value', 'desc'), limit(top));
...
export async function fetchMyRank(
  ...
  const q = query(scoresCol(category, metric, window), where('value', '>', myValue));
...
export async function fetchOvertakeTarget(
  ...
  const q = query(
    scoresCol(category, metric, window),
    where('value', '>', myValue),
    orderBy('value', 'asc'),
    limit(1),
  );
...
export function myValueFor(
  ...
  if (metric === 'global_correct') {
    const t = correctTotals(scoped, startMs);
    return window === 'week' ? t.week : t.all;
  }
  const b = bestScore(scoped, metric as GameMode, startMs);
